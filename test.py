from BitVector import *

Sbox = (
    ,
)

def g(w, r_c):
    print("got:", w)
    w = w[2:] + w[:2]
    print("shifted:", w)
    b_w = BitVector(hexstring=w)
    sub_w = BitVector(size = 0)
    for i in range(4):
        sub_w += BitVector(intVal = Sbox[b_w[i*8:i*8+8].intValue()], size = 8)
    print(sub_w.get_bitvector_in_hex())
    sub_w[0:8] ^= r_c

    print(sub_w.get_bitvector_in_hex())
    return sub_w

def addRoundKey(hex_plain_text, hex_key):
    state_mat = []
    round_key_mat = []

    for i in range(0, 8, 2):
        state_row = []
        round_key_row = []
        for j in range(4):
            state_row.append(BitVector(hexstring = hex_plain_text[i+j*8:i+j*8+2]))
            round_key_row.append(BitVector(hexstring=hex_key[i+j*8:i+j*8+2]))
        
        state_mat.append(state_row)
        round_key_mat.append(round_key_row)
    
    new_state_mat = []
    for i in range(4):
        new_state_row = []
        for j in range(4):
            new_state_row.append(state_mat[i][j]^round_key_mat[i][j])
        new_state_mat.append(new_state_row)

    # printMatrix(new_state_mat)
    # printMatrix(state_mat)
    # printMatrix(round_key_mat)
    substitutionByte(new_state_mat)


def substitutionByte(state_mat):
    for i in range(4):
        for j in range(4):
            state_mat[i][j] = BitVector(intVal = Sbox[state_mat[i][j].intValue()], size = 8)

    # printMatrix(state_mat)
    shiftRow(state_mat)


def shiftRow(state_mat):
    for i in range(4):
        state_mat[i] = state_mat[i][i:] + state_mat[i][:i]

    # printMatrix(state_mat)
    mixColumn(state_mat)


def mixColumn(state_mat):
    fixed_mat = [
        [BitVector(hexstring="02"), BitVector(hexstring="03"), BitVector(hexstring="01"), BitVector(hexstring="01")],
        [BitVector(hexstring="01"), BitVector(hexstring="02"), BitVector(hexstring="03"), BitVector(hexstring="01")],
        [BitVector(hexstring="01"), BitVector(hexstring="01"), BitVector(hexstring="02"), BitVector(hexstring="03")],
        [BitVector(hexstring="03"), BitVector(hexstring="01"), BitVector(hexstring="01"), BitVector(hexstring="02")]
    ]

    new_state_mat = []
    for i in range(4):
        new_state_row = []
        for j in range(4):
            new_state_row.append(BitVector(intVal=0, size=8))
        new_state_mat.append(new_state_row)

    for i in range(4):
        for j in range(4):
            for k in range(4):
                prod = fixed_mat[i][k].gf_multiply_modular(state_mat[k][j], BitVector(intVal=0x11b, size=9), 8)
                new_state_mat[i][j] ^= prod

    printMatrix(new_state_mat)

def printMatrix(mat):
    for i in range(4):
        for j in range(4):
            print(mat[i][j].get_bitvector_in_hex(), end=" ")
        print()

    

text = "54776f204f6e65204e696e652054776f"
key = "5468617473206d79204b756e67204675"
# g_w = g(key[-8:], BitVector(intVal=0x01, size=8))
# res = BitVector(size = 0)
# for i in range(4):
#     w = BitVector(hexstring=key[i*8:i*8+8])
#     # print("w:", i, ":", w.get_bitvector_in_hex())
#     part = g_w ^ w
#     print("part:", i, ":", part.get_bitvector_in_hex())
#     res += part;
#     g_w = part

# s = res.get_bitvector_in_hex()
# print(s)
# print(s[-8:])
# print(key)

addRoundKey(text, key)
# i = 1
# j = 2
# print(text[i*8+j*32:i*8+j*32+8])
# print(BitVector(hexstring = text[i*8+j*32:i*8+j*32+8]).get_bitvector_in_hex())
